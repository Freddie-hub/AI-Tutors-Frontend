  ...
  const { formattedHtml, toc } = useMemo(() => {
    ...
    headings.forEach((h) => {
      ...
      if (lvl === 1) {
        ...
        h.replaceWith(newH);
        h2 += 1; h3 = 0; h4 = 0;
        const id = `section-${h2}-${slugify(newH.textContent || '')}`;
        newH.id = id;
        newH.classList.add('mt-8','mb-3','scroll-mt-24','border-b','border-white/10','pb-1');
        newH.innerHTML = `<span class=\"text-blue-400 mr-2\">${h2}.</span>${newH.innerHTML}`;
        tocItems.push({ id, title: newH.textContent || `Section ${h2}`, level: 2 });
        return;
      }
      if (lvl === 2) {
        h2 += 1; h3 = 0; h4 = 0;
        const id = `section-${h2}-${slugify(h.textContent || '')}`;
        h.id = id;
        h.classList.add('mt-8','mb-3','scroll-mt-24','border-b','border-white/10','pb-1');
        h.innerHTML = `<span class=\"text-blue-400 mr-2\">${h2}.</span>${h.innerHTML}`;
        tocItems.push({ id, title: h.textContent || `Section ${h2}`, level: 2 });
        return;
      }
      if (lvl === 3) {
        h3 += 1; h4 = 0;
        const id = `section-${h2}-${h3}-${slugify(h.textContent || '')}`;
        h.id = id;
        h.classList.add('mt-6','mb-2','scroll-mt-24');
        h.innerHTML = `<span class=\"text-sky-300 mr-2\">${h2}.${h3}</span> ${h.innerHTML}`;
        tocItems.push({ id, title: h.textContent || `Section ${h2}.${h3}`, level: 3 });
        return;
      }
      if (lvl === 4) {
        h4 += 1;
        const id = `section-${h2}-${h3}-${h4}-${slugify(h.textContent || '')}`;
        h.id = id;
        h.classList.add('mt-4','mb-1.5','scroll-mt-24');
        h.innerHTML = `<span class=\"text-teal-300 mr-2\">${h2}.${h3}.${h4}</span> ${h.innerHTML}`;
        tocItems.push({ id, title: h.textContent || `Section ${h2}.${h3}.${h4}`, level: 4 });
      }
    });
    ...
    container.querySelectorAll('table').forEach((table) => {
      ...
      rows.forEach((tr, idx) => { if (idx % 2 === 1) tr.classList.add('bg-white/[0.03]'); });
    });
    ...
    container.querySelectorAll('blockquote').forEach((bq) => bq.classList.add('border-l-2','border-blue-500/40','pl-4','italic','text-white/80','my-4'));
    container.querySelectorAll('hr').forEach((hr) => hr.classList.add('my-6','border-white/10'));
    container.querySelectorAll('pre').forEach((pre) => pre.classList.add('bg-white/5','rounded-lg','p-3','overflow-x-auto','my-4'));
    container.querySelectorAll('code').forEach((code) => code.classList.add('text-blue-200'));
