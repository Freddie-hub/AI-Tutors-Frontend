...
import curriculumData from '@/components/GCSEStudent/gcse_curriculum_simple.json';
...
export default function LessonFormModal({ open, onClose }: Props) {
  ...
  const curriculum = useMemo(() => curriculumData as CurriculumYear[], []);

  // State for selections
  const [selectedYearIndex, setSelectedYearIndex] = useState<number>(6); // Default to Year 7
  ...
  const currentYear = useMemo(() => curriculum[selectedYearIndex], [curriculum, selectedYearIndex]);
  const currentSubject = useMemo(() => currentYear?.subjects[selectedSubjectIndex], [currentYear, selectedSubjectIndex]);
  ...
  const grade = currentYear?.programme || 'Year 7';
  ...
  const curriculumContext = useMemo(() => {
    ...
    const contentItems = currentStrand.subtopics || currentStrand.objectives || [];
    const contentLabel = currentStrand.subtopics ? 'Subtopics' : 'Objectives';
...
${contentLabel} from Cambridge curriculum:
${contentText}

Note: These are the official ${contentLabel.toLowerCase()} from the Cambridge curriculum. Structure your lesson around these while maintaining pedagogical flow.`;
  }, [currentStrand]);

  // Ensure defaults are valid when year changes
  useEffect(() => {
    if (currentYear && currentYear.subjects.length > 0) {
      // Reset subject to first if current index is invalid
      if (selectedSubjectIndex >= currentYear.subjects.length) {
        setSelectedSubjectIndex(0);
        setSelectedStrandId('');
      }
    }
  }, [selectedYearIndex, currentYear, selectedSubjectIndex]);
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                >
                  {curriculum.map((y, idx) => (
                    <option key={idx} value={idx}>
                      Year {y.year_number} - {y.programme.split(' - ')[0].replace('Cambridge ', '')}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                >
                  {currentYear?.subjects.map((s, idx) => (
                    <option key={idx} value={idx}>
                      {s.name}
                    </option>
